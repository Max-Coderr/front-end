...
import HeaderItem from "../../components/components/Header/Header.tsx";
import Footer from "../../components/components/Footer/Footer.tsx";
import Section from "../../components/components/Section.tsx";
import BookItem from "../../components/components/Book/BookItem.tsx";
import { MOCK_NEWS } from "../../mockData.ts";
    ...
    useEffect(() => {
        ...
        async function getNews() {
            try {
                ...
                const found = MOCK_NEWS.find((n) => n.id === Number(id));
                if (found) {
                    setNews(found);
                    setSimilar(MOCK_NEWS.filter((n) => n.id !== Number(id)).slice(0, 3));
                } else {
                    setNews(null);
                }
            }
        ...
        <div className="bg-[#202020] min-h-screen text-white">
            ...
            <div className="container py-32 text-center">
                ...
                <button
                    ...
                    className="text-[#F97316] text-[16px] font-medium cursor-pointer bg-transparent border-none outline-none hover:underline"
                >
    ...
    return (
        <div className="bg-[#202020] min-h-screen text-white">
            ...
            <div className="flex gap-3 w-[1374px] h-[44px] items-center pl-[30px] ml-[34px]">
                ...
                <h4 className="text-[#6D7274] font-medium mb-1 cursor-pointer" onClick={() => router.push("/news")}>Yangiliklar</h4>
                <img src="/NewsImage/icon7.svg" alt="icon" className="w-2 h-2 mt-2 mb-[2px]"/>
                <h4 className="text-white font-medium mb-1 truncate max-w-[600px]">{news.title}</h4>
            </div>
                            ...
                            <div className="flex flex-wrap justify-between items-center gap-4 border-t border-b border-[#1F272A] py-4 mt-8">
                                <div className="flex items-center gap-4">
                                    {["/twitter.svg", "/facebook 01.svg", "/telegram.svg", "/log-in.svg"].map((s) => (
                                        <button key={s} className="opacity-60 hover:opacity-100 cursor-pointer bg-transparent border-none">
                                ...
                                <div className="flex items-center gap-4 text-[#9DA1A3] text-[13px]">
                                    <button className="flex items-center gap-1.5 bg-[#0D0F10] border border-[#1F272A] hover:bg-[#262A2D] text-white px-3 py-1.5 rounded-[8px] text-[12px] cursor-pointer">
                                        ...
                                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.684 10.742l4.628-2.2a3 3 0 11.83 1.666l-4.628 2.2a3 3 0 01-.83-1.666zM13.316 13.258l-4.628 2.2a3 3 0 10.83 1.666l4.628-2.2a3 3 0 00-.83-1.666z"/>
                                        </svg>
                                    </button>
                                </div>
                            </div>
                        ...
                        {similar.length > 0 && (
                            <div className="mt-8 mb-12">
                                <div className="flex justify-between items-center mb-5">
                                    <h2 className="text-[18px] font-bold font-sans text-[#FCFCFC]">O'xshash yangiliklar</h2>
                                    <button
                                        onClick={() => router.push("/news")}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                                    {similar.map((item) => (
                                        <div
                                            ...
                                            className="bg-[#1A1D1F] border border-[#1F272A] rounded-[10px] p-4 cursor-pointer hover:border-[#F97316] transition-all"
                                        >
                                            <img src={item.image} alt="" className="w-full h-[120px] object-cover rounded-[6px] mb-3"/>
